...
end                                          %Carl: Setting up default parameters, no less then 2 but no more than
                                             %      6. Setting up one by one
...
if m~=size(data,1)                                    %Carl: M = bigger size of matrix?or the number of points in dataset
...
if m<4
    ...
    resid=zeros(N,1);
    vi=ones(N,1);
end                                                     %Carl: m = dimension of points, that is to say 4 dimension or under using 
    ...
    switch critFun                                     %Carl: critFun~~
        ...
        case 0
            ...
            if mTranspose
                ...
                [U,~,V]=svd(C);
                Ri=V*U';
                if det(Ri)<0                           %Carl: Ri none negetive?
                    V(:,end)=-V(:,end);
                    Ri=V*U';
                end
                Ti=mem'-Ri*med;
